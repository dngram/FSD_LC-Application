...
export default function Home() {
  ...
  const [P_Receipt, setP_Receipt] = useState<File | null>(null);
  const [Prev_LC, setPrev_LC] = useState<File | null>(null);
  const [Pro_Deg_Cer, setPro_Deg_Cer] = useState<File | null>(null);
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (P_Receipt) formData.append("P_Receipt", P_Receipt);
    if (Prev_LC) formData.append("Prev_LC", Prev_LC);
    if (Pro_Deg_Cer) formData.append("Pro_Deg_Cer", Pro_Deg_Cer);
...
<label htmlFor="P_Receipt">Upload Payment Receipt</label>
          <input
            id="P_Receipt"
            type="file"
            accept=".pdf"
            onChange={(e) => setP_Receipt(e.target.files?.[0] || null)}
          />

          <label htmlFor="Prev_LC">Upload Previous LC</label>
          <input
            id="Prev_LC"
            type="file"
            accept=".pdf"
          ...
          <label htmlFor="Pro_Deg_Cer">Upload Provisional Degree Certificate:</label>
          <input
            id="Pro_Deg_Cer"
            type="file"
            accept=".pdf"
            onChange={(e) => setPro_Deg_Cer(e.target.files?.[0] || null)}
          />

          <label htmlFor="Marksheet">Upload Student Marksheet:</label>
          <input
            id="Marksheet"
            type="file"
            accept=".pdf"
